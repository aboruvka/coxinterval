...
```
The check should give only one note on the package size. CPLEX libraries are static, which inflates the size of the shared object file `libs/coxinterval.so`.
