...
coxaalenic <- function(formula, data = parent.frame(), subset, init = NULL,
  ...
  names(icfit$coef) <- names(mf)[iprp]
  var <- matrix(icfit$var, nprp)
  rownames(var) <- colnames(var) <- names(mf)[iprp]
  bhaz <- cbind(time$int[, 2], t(matrix(icfit$bhaz, nadd + 1)))
  ...
  fit <- list(call = cl, n = n, p = nprp, coef = icfit$coef, var = var,
              bhaz = bhaz, init = init, iter = icfit$iter,
              loglik = n * icfit$loglik[1:(icfit$iter + 1)],
              fenchel = icfit$fenchel, maxnorm = icfit$maxnorm,
              cputime = icfit$cputime, control = control, rcfit = rcfit,
